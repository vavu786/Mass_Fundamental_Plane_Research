...
def main():
    ...
    for fig_ax in [*np.reshape(mfp_ax, -1), *np.reshape(mfp_ax_allz, -1), *np.reshape(RM_sigmaM_ax, -1), mfp_fig, mfp_fig_allz, RM_sigmaM_fig]:
        getattr(fig_ax, "set_facecolor")(bkg_color)
    ...
    for i, MFP_axis, logR_M_axis, logsigma_M_axis, R_logSigma_axis, f_dm_lmass_axis, zmin, zmax in zip(
        ...
        MFP_axis.text(-0.4, -3.1, f"$\gamma_z$ = {-abs(round(fit_pms[0], 3))} $\pm$ {std_devs[-1]}", size="x-large")
        MFP_axis.text(-0.4, -3.3, f"scatter = {round(scatters[-1], 3)} $\pm$ {round(scatter_conf_int.standard_error, 3)}", size="x-large")
